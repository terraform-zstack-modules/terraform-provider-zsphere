...
TEST_DIR="${PROJECT_ROOT}/test/template-instance"
...
Environment Variables:
  ...
  TF_VAR_template_uuid             Template VM UUID for testing
  TF_VAR_template_name             Template VM name for testing
  TF_VAR_port_group_name           Port group name for testing

Examples:
  $0 build
  $0 all
  $0 basic
  $0 run create_basic_instance_from_template
EOF
}
...
main() {
    ...
    case "${command}" in
        ...
        config) build_provider; run_config_tests ;;
